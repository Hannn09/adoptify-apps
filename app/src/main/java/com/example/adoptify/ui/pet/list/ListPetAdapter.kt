...
class ListPetAdapter(private val listPet: List<DataItem>) : RecyclerView.Adapter<ListPetAdapter.ListViewHolder>(){
        ...
        fun bind(pet: DataItem) {
            binding.apply {
                Glide.with(itemView.context)
                    ...
                    .into(itemImage)
                petName.text = pet.nama.split(" ").joinToString(" ") { it.capitalize() }
                petRas.text = pet.ras.split(" ").joinToString(" ") { it.capitalize() }
                petYears.text = pet.usia.split(" ").joinToString(" ") { it.capitalize() }
                ...
                itemView.setOnClickListener {
                    ...
                    intent.putExtra(EXTRA_USER, pet.uid)
